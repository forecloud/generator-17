...
//  @17: 修改2.2，添加默认要导入的jar包
        innerClass.addJavaDocLine("import cn.xdb.core.database.ColumnName;\n" +
                "import cn.xdb.core.database.TableName;\n" +
                "import lombok.Data;\n" +
...
//  @17: 修改2.3，添加类注释和类注解
        //说明1：如果在xml中配置关闭注释，注解也不会生成
        StringBuilder sb = new StringBuilder();
        String url = JDBCConnectionConfiguration.getURL();
        innerClass.addJavaDocLine("/**"); //$NON-NLS-1$
        innerClass.addJavaDocLine(" * 类对应的表： "); //$NON-NLS-1$
        sb.append("   " + url.substring(url.lastIndexOf('/') + 1)+"."+introspectedTable.getFullyQualifiedTable());
        innerClass.addJavaDocLine(sb.toString());
//  注解部分：以下
        innerClass.addJavaDocLine(" */"); //$NON-NLS-1$
        innerClass.addJavaDocLine("@Data\n" +
                "@TableName(name = \""+ this.tableName +"\",oldName = \""+ introspectedTable.getFullyQualifiedTable() +"\")");
...
//  @17：修改2.8，增加属性注释和注解
        ...
        field.addJavaDocLine("@ColumnName(name = \""+ introspectedColumn.getActualColumnName() +"\"," +
                "oldName = \"" + introspectedColumn.getColumnName() +"\")");
